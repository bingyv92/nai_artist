...
图片类型选择（主动判断）
photo：代表你自己（角色形象）的自拍或你所见的场景。生成时，你的默认形象作为参考。专注于姿势、氛围、场景（例如：“在公园长椅上托腮发呆”“坐在窗边对你眨眼”）。

drawing：不自动绑定你的外貌，可用于画用户指定的其他角色、OC、二创或非你自己的对象。也可像表情包一样使用，或绘制幻想场景、抽象概念。
...
→ 主动生成本应景的图片：比如你捧着蛋糕，或你站在雪中伸手接雪花。'''
    ...
    async def execute(
        ...
        content: Annotated[
            str,
            '''首先理解原理：生图是按关键词来的，有哪些关键词就会有什么元素会被呈现。用关键词词组刻画画面内容——详细的，不单调的场景（需要将背景和场景中的元素都描述出来，不然背景就是简单的色块）、足够准确的物体肢体或者人物（可以只出现双腿，双脚之类的，而不是全身）、具体的光影、情感或者说表情，注意不是描述，而是刻画。给出的描述要姿势生动，画面条理和肢体关系清楚，尽可能使用和指明不同角度的镜头，特写，（使用时要考虑画面关系）这样会更生动，不要出现类似于拍囚犯的全身照。photo 模式下会把你的人设作为同等级参考交给翻译模型，并按取景选择性吸收，描述画面出现的元素即可；若是在画其他角色，只能写用户已明确提供的设定，不要补完未说明的细节。''',
        ],
    ) -> tuple[bool, str]:
        """执行生图并发送。
