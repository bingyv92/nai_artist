...
→ 主动生成本应景的图片：比如你捧着蛋糕，或你站在雪中伸手接雪花。'''
    ...
    async def execute(
        ...
        content: Annotated[
            str,
            '''首先理解原理：生图是按关键词来的，有哪些关键词就会有什么元素会被呈现。用关键词词组刻画画面内容——详细的，不单调的场景、足够准确的物体肢体或者人物（可以只出现双腿，双脚之类的，而不是全身）、具体的光影、情感或者说表情，注意不是描述，而是刻画。给出的描述要姿势生动，画面条理和肢体关系清楚，尽可能使用和指明不同角度的镜头，特写，这样会更生动，不要出现类似于拍囚犯的全身照。photo 模式下会把你的人设作为同等级参考交给翻译模型，并按取景选择性吸收，描述画面出现的元素即可；若是在画其他角色，只能写用户已明确提供的设定，不要补完未说明的细节。''',
        ],
    ) -> tuple[bool, str]:
        """执行生图并发送。
